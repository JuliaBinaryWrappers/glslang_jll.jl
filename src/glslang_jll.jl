...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import glslang_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("glslang_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "glslang_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"glslang", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "glslang_jll", "src", "glslang_jll.jl"))
        @info("glslang_ll dev'ed out to /depot/dev/glslang_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("6fd5517d-459c-5c5a-9b1a-c968b4e37a81"))
platforms = [Pkg.Artifacts.unpack_platform(e, "glslang", artifacts_toml) for e in artifacts["glslang"]]
...
if best_platform === nothing
    @debug("Unable to load glslang; unsupported platform $(triplet(platform_key_abi()))")
else
